...
public class StudentiDao implements IStudentiDao {
	...
	public Studente updateStu(Studente stu) {
		...
		logger.info(String.format("Aggiornato studente con matricola %d", stu.getStuMatricola()));
		return stu1;
	}
	...
	public Studente createStu(String stuCognome, String stuNome, 
		...
		if (stuCognome == null || stuNome == null || stuSesso == null || stuIscrizione == null
				|| stuCorsoDiLaurea == null)
			throw new NullPointerException();
		// 2) i parametri stuCognome, stuNome devono avere
		// almeno due caratteri non blank
		if (stuCognome.trim().length() < 2 || stuNome.trim().length() < 2)
			throw new IllegalArgumentException();
		// 3) il parametro stuSesso può essere solo M o F
		if (!stuSesso.equals("M") && !stuSesso.equals("F"))
			throw new IllegalArgumentException();
		int maxStuMatricola = session.createQuery("SELECT max(s.stuMatricola) FROM Studente s", Integer.class)
				.getSingleResult();
		...
		session.persist(stu);
		logger.info(String.format("Creato stu con matricola %d", stuMatricola));
		return stu;
	}

	@Override
	public Studente createIscrizione(int stuMatricola, int matId, AnnoAccademico matAnnoAccademico) {
		// VERIFICA FORMALE
		...
		if (stu != null) {
			...
			Materia mat = session.createQuery("SELECT m FROM Materia m" + " WHERE m.id = :id", Materia.class)
					.setParameter("id", id).getSingleResult();
			// PRIMA di aggiungere la materia
			// verifico il vincolo sui dati (vedi sopra)
			int stuAnnoCorso = matAnnoAccademico.getAnno() - stu.getStuIscrizione().getAnno() + 1;
			...
			session.persist(stu);
			logger.info(String.format("creata iscrizione %d %d %s", stuMatricola, matId, matAnnoAccademico));
		}
		return stu;
	}

	@Override
	public List<Studente> readAllStu() {
		List<Studente> stus = session.createQuery("FROM Studente s", Studente.class).getResultList();
		return stus;
	}
}
